...
let ChatsGateway = class ChatsGateway {
    constructor(chattingModel, socketModel) {
        ...
        this.logger.log('Starting Chat Gateway');
    }
    afterInit() {
        this.logger.log(`🚀 Gateway initialized`);
    }
    handleConnection(socket) {
        this.logger.log(`connected to ${socket.id} ${socket.nsp.name}`);
    }
    async handleDisconnect(socket) {
        ...
        this.logger.log(`disconnected to ${socket.id} ${socket.nsp.name}`);
    }
    ...
    async handleSubmitChat(chat, socket) {
        const socketObj = await this.socketModel.findOne({
            id: socket.id,
        });
